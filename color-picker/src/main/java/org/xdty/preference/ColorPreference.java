...
public class ColorPreference extends Preference implements ColorPickerSwatch
    ...
    public ColorPreference(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable
                .ColorPreference, 0, 0);
        try {
            int id = a.getResourceId(R.styleable.ColorPreference_colorpicker_colors, R.array.default_rainbow);
            if (id != 0) {
                mColors = getContext().getResources().getIntArray(id);
            }
            id = a.getResourceId(R.styleable.ColorPreference_colorpicker_dialogTitle, 0);
            if (id != 0) {
                mTitle = a.getResourceId(R.styleable.ColorPreference_colorpicker_dialogTitle,
                        R.string.color_picker_default_title);
            }
            mColumns = a.getInt(R.styleable.ColorPreference_colorpicker_columns, 5);
            mMaterial = a.getBoolean(R.styleable.ColorPreference_colorpicker_material, true);
            mBackwardsOrder = a.getBoolean(R.styleable.ColorPreference_colorpicker_backwardsOrder, true);
            mStrokeWidth = a.getInt(R.styleable.ColorPreference_colorpicker_strokeWidth, 0);
            mStrokeColor = a.getColor(R.styleable.ColorPreference_colorpicker_strokeColor, 0xff000000);
        } finally {
            a.recycle();
        }
    }
